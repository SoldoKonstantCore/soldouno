...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Soldo.getBlockchainProcessor().isScanning() ?
                    Math.max(Soldo.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Soldo.getBlockchainProcessor().getMinRollbackHeight();
            if (height < minRollBackHeight) {
                throw new IllegalArgumentException("Historical data as of height " + height + " not available.");
            }
        }
        if (height > Soldo.getBlockchain().getHeight()) {
            throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Soldo.getBlockchain().getHeight());
        }
    }
    ...
    private boolean doesNotExceed(int height) {
        return Soldo.getBlockchain().getHeight() <= height && ! (isPersistent() && Soldo.getBlockchainProcessor().isScanning());
    }
