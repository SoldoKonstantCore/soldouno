...
import soldo.util.CountingInputReader;
import soldo.util.CountingInputStream;
import soldo.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Soldo.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Soldo.getBlockchain().getHeight() - 60) {
    ...
    public void blacklist(String cause) {
        blacklistingTime = Soldo.getEpochTime();
    ...
    void connect() {
        lastConnectAttempt = Soldo.getEpochTime();
        try {
