...
public abstract class TransactionType {
    ...
    abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws SOLException.NotValidException;

    abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws SOLException.NotValidException;

    abstract void validateAttachment(Transaction transaction) throws SOLException.ValidationException;
    ...
    public static abstract class Payment extends TransactionType {
        ...
        public static final TransactionType ORDINARY = new Payment() {
            ...
            @Override
            Attachment.EmptyAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws SOLException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            Attachment.EmptyAttachment parseAttachment(JSONObject attachmentData) throws SOLException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            void validateAttachment(Transaction transaction) throws SOLException.ValidationException {
                if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_centesimo) {
                    throw new SOLException.NotValidException("Invalid ordinary payment");
                }
            }
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public final static TransactionType ARBITRARY_MESSAGE = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws SOLException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0) {
                    throw new SOLException.NotValidException("Invalid arbitrary message: " + attachment.getJSONObject());
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) { // NO MESSAGES TO GENESIS
                    throw new SOLException.NotValidException("Sending messages to Genesis not allowed.");
                }
            }
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            private final Fee ALIAS_FEE = new Fee.SizeBasedFee(2 * Constants.ONE_SOL, 2 * Constants.ONE_SOL, 32) {
                @Override
            ...
            @Override
            Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws SOLException.NotValidException {
                return new Attachment.MessagingAliasAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasAssignment parseAttachment(JSONObject attachmentData) throws SOLException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws SOLException.ValidationException {
                ...
                if (attachment.getAliasName().length() == 0
                    ...
                    throw new SOLException.NotValidException("Invalid alias assignment: " + attachment.getJSONObject());
                }
                String normalizedAlias = attachment.getAliasName().toLowerCase();
                for (int i = 0; i < normalizedAlias.length(); i++) {
                    if (Constants.ALPHABET.indexOf(normalizedAlias.charAt(i)) < 0) {
                        throw new SOLException.NotValidException("Invalid alias name: " + normalizedAlias);
                    }
                }
                Alias alias = Alias.getAlias(normalizedAlias);
                if (alias != null && alias.getAccountId() != transaction.getSenderId()) {
                    throw new SOLException.NotCurrentlyValidException("Alias already owned by another account: " + normalizedAlias);
                }
            }
        ...
        public static final TransactionType ALIAS_DELETE = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasDelete parseAttachment(final ByteBuffer buffer, final byte transactionVersion) throws SOLException.NotValidException {
                return new Attachment.MessagingAliasDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasDelete parseAttachment(final JSONObject attachmentData) throws SOLException.NotValidException {
            ...
            @Override
            void validateAttachment(final Transaction transaction) throws SOLException.ValidationException {
                final Attachment.MessagingAliasDelete attachment
                        = (Attachment.MessagingAliasDelete) transaction.getAttachment();
                final String aliasName = attachment.getAliasName();
                if (aliasName == null || aliasName.length() == 0) {
                    throw new SOLException.NotValidException("Missing alias name");
                }
                final Alias alias = Alias.getAlias(aliasName);
                if (alias == null) {
                    throw new SOLException.NotCurrentlyValidException("No such alias: " + aliasName);
                } else if (alias.getAccountId() != transaction.getSenderId()) {
                    throw new SOLException.NotCurrentlyValidException("Alias doesn't belong to sender: " + aliasName);
                }
            }
        ...
        public static final Messaging ACCOUNT_INFO = new Messaging() {
            ...
            private final Fee ACCOUNT_INFO_FEE = new Fee.SizeBasedFee(Constants.ONE_SOL, 2 * Constants.ONE_SOL, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws SOLException.NotValidException {
                return new Attachment.MessagingAccountInfo(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountInfo parseAttachment(JSONObject attachmentData) throws SOLException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws SOLException.ValidationException {
                ...
                if (attachment.getName().length() > Constants.MAX_ACCOUNT_NAME_LENGTH
                    ...
                    throw new SOLException.NotValidException("Invalid account info issuance: " + attachment.getJSONObject());
                }
            }
