...
import static soldo.Genesis.CREATOR_ID;
...
final class TransactionImpl implements Transaction {
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws SOLException.NotValidException {
        ...
        if ( builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null) ) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Soldo.getBlockchain().getHeight());
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || Soldo.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
