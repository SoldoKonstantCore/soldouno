...
import soldo.db.DbClause;
import soldo.db.DbIterator;
import soldo.db.DbKey;
import soldo.db.DbUtils;
import soldo.db.DerivedDbTable;
import soldo.db.VersionedEntityDbTable;
import soldo.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class AccountInfo {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_info "
                ...
                pstmt.setInt(++i, Soldo.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Soldo.getBlockchain().getHeight();
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Soldo.getBooleanProperty("soldo.enablePublicKeyCache") ?
    ...
    static {
        if (publicKeyCache != null) {
            Soldo.getBlockchainProcessor().addListener(block -> {
            ...
            Soldo.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Soldo.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Soldo.getBlockchain().getHeight()) {
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Soldo.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Soldo.getBlockchain().getHeight() - 1) {
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.SOLDO_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.SOLDO_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_SOLDO_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_SOLDO_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
