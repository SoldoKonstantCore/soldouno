 ...
 *
 *   java -cp "classes:lib/*:conf" soldo.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dsoldo.runtime.mode=desktop soldo.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "soldo.testDb" : "soldo.db";
        String dbType = Soldo.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Soldo.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Soldo.getDbDir(Soldo.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Soldo.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Soldo.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Soldo.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "soldo.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "soldo.mv.db");
            if (!dbFile.exists()) {
        ...
        try {
            ...
            switch (phase) {
                ...
                case 1:
                    ...
                    //
                    File newFile = new File(dbDir, "soldo.h2.db");
                    if (newFile.exists()) {
                        if (!newFile.delete()) {
                            Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                        }
                    } else {
                        newFile = new File(dbDir, "soldo.mv.db");
                        if (newFile.exists()) {
                            if (!newFile.delete()) {
                                Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                            }
