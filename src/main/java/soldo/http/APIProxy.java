...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Soldo.getBooleanProperty("soldo.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Soldo.getIntProperty("soldo.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Soldo.getStringProperty("soldo.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Soldo.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        blacklistedPeers.put(host, Soldo.getEpochTime() + blacklistingPeriod);
