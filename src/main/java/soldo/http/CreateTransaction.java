...
import static soldo.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static soldo.http.JSONResponses.INCORRECT_DEADLINE;
import static soldo.http.JSONResponses.INCORRECT_EC_BLOCK;
import static soldo.http.JSONResponses.INCORRECT_WHITELIST;
import static soldo.http.JSONResponses.MISSING_DEADLINE;
import static soldo.http.JSONResponses.MISSING_SECRET_PHRASE;
import static soldo.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
        ...
        long feeNQT = Soldo.softMG().getFixedFee(amountNQT);
        ...
        if (ecBlockId != 0 && ecBlockId != Soldo.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Soldo.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Soldo.newTransactionBuilder(publicKey, amountNQT, feeNQT,
