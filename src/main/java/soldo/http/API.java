...
import static soldo.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static soldo.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static soldo.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static soldo.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = Soldo.getStringProperty("soldo.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Soldo.getIntProperty("soldo.maxAPIRecords");
    static final boolean enableAPIUPnP = Soldo.getBooleanProperty("soldo.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Soldo.getIntProperty("soldo.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Soldo.getBooleanProperty("soldo.apiServerCORS");
    private static final String forwardedForHeader = Soldo.getStringProperty("soldo.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Soldo.getStringListProperty("soldo.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Soldo.getStringListProperty("soldo.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Soldo.getStringListProperty("soldo.allowedBotHosts");
        ...
        boolean enableAPIServer = Soldo.getBooleanProperty("soldo.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Soldo.getIntProperty("soldo.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Soldo.getIntProperty("soldo.apiServerSSLPort");
            final String host = Soldo.getStringProperty("soldo.apiServerHost");
            disableAdminPassword = Soldo.getBooleanProperty("soldo.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            final boolean enableSSL;
            String keyStoreType = Soldo.getStringProperty("soldo.keyStoreType");
            String keyStorePassword = Soldo.getStringProperty("soldo.keyStorePassword", null, true);
            boolean enableSSLProperty = Soldo.getBooleanProperty("soldo.apiSSL");
            boolean enforceSSL = Soldo.getBooleanProperty("soldo.apiServerEnforceSSL", true);
            boolean listeningLocally = host.equals("127.0.0.1");
            String keyStorePathProperty = Soldo.getStringProperty("soldo.keyStorePath");

            //
            // Enforce SSL if API is not for local usage
            //
            if (enforceSSL && !enableSSLProperty && !listeningLocally) {
                keyStorePassword = API.adminPassword.length()==0?"DefaultSoldoSSLPassword":API.adminPassword;
                SSLCert cert = new SSLCert(keyStorePassword);
                String keyStorePath = Paths.get(Soldo.getUserHomeDir()).resolve(Paths.get("soldo.default.ks")).toString();
                ...
                keyStoreType="JKS";
                keyStorePathProperty = "soldo.default.ks";
                enableSSL = true;
            } else {
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Soldo.getUserHomeDir()).resolve(Paths.get(keyStorePathProperty)).toString();
                ...
                List<String> ciphers = Soldo.getStringListProperty("soldo.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Soldo.getStringProperty("soldo.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Soldo.getStringProperty("soldo.apiWelcomeFile")});
            }

            String javadocResourceBase = Soldo.getStringProperty("soldo.javadocResourceBase");
        ...
        //    apiHandler.addServlet(DbShellServlet.class, "/dbshell");
            ...
            if (Soldo.getBooleanProperty("soldo.apiFrameOptionsSameOrigin")) {
