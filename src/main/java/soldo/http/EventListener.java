...
import soldo.TransactionProcessor;
 ...
 * has not been received within soldo.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by soldo.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Soldo.getIntProperty("soldo.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Soldo.getIntProperty("soldo.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Soldo.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Soldo.getTransactionProcessor();
    ...
    /** Soldo event listeners */
    private final List<SoldoEventListener> SoldoEventListeners = new ArrayList<>();
 ...
 Soldo event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Soldo event registrations
     ...
     *
     * @param   eventRegistrations      Soldo event registrations
     * @throws  EventListenerException  Invalid Soldo event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<SoldoEventListener> it = SoldoEventListeners.iterator();
                while (it.hasNext()) {
                    SoldoEventListener listener = it.next();
                ...
                if (addListener) {
                    SoldoEventListener listener = new SoldoEventListener(event);
                    listener.addListener();
                    SoldoEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void removeEvents(List<EventRegistration> eventRegistrations) {
        ...
        try {
            ...
            //
            if (SoldoEventListeners.isEmpty())
                deactivateListener();
        } finally {
            lock.unlock();
        }
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            SoldoEventListeners.forEach(SoldoEventListener::removeListener);
        } finally {
            lock.unlock();
        }
    ...
    /**
     * Soldo event listener
     */
    private class SoldoEventListener {

        /** Event handler */
        private final SoldoEventHandler eventHandler;

        /**
         * Create the Soldo event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public SoldoEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Soldo listener for this event
         */
        ...
        /**
         * Remove the Soldo listener for this event
         */
        ...
        /**
         * Return the hash code for this Soldo event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Soldo events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof SoldoEventListener) &&
                    eventHandler.equals(((SoldoEventListener)obj).eventHandler));
        }

        /**
         * Soldo listener event handler
         */
        private abstract class SoldoEventHandler {
            ...
            /** Soldo listener event */
            protected final Enum<? extends Enum> event;

            /**
             * Create the Soldo event handler
             *
             * @param   eventRegistration   Event registration
             */
            public SoldoEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof SoldoEventHandler) &&
                                        owner == ((SoldoEventHandler)obj).owner &&
                                        accountId == ((SoldoEventHandler)obj).accountId &&
                                        event == ((SoldoEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends SoldoEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends SoldoEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends SoldoEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends SoldoEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Soldo listener event
        ...
        /**
         * Return the Soldo listener event
         *
         * @return                  Soldo listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
