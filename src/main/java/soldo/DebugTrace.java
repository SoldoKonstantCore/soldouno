...
public final class DebugTrace {
    ...
    static final String QUOTE = Soldo.getStringProperty("soldo.debugTraceQuote", "\"");
    static final String SEPARATOR = Soldo.getStringProperty("soldo.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Soldo.getBooleanProperty("soldo.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Soldo.getStringListProperty("soldo.debugTraceAccounts");
        String logName = Soldo.getStringProperty("soldo.debugTraceLog");
        ...
        Soldo.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Soldo.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Soldo.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Soldo.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
        return debugTrace;
    }
   ...
   private void traceBeforeAccept(Block block) {
    ...
    private Map<String, String> getValues(long accountId, boolean unconfirmed) {
        ...
        map.put("timestamp", String.valueOf(Soldo.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Soldo.getBlockchain().getHeight()));
