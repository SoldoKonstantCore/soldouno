...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Soldo.getBooleanProperty("soldo.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Soldo.getBooleanProperty("soldo.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Soldo.getIntProperty("soldo.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Soldo.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Soldo.getEpochTime()), 0, -1, "")) {
        ...
        try {
            for (Transaction transaction : transactions) {
                ...
                try {
                    ...
                    if (trx.getSender() == Genesis.CREATOR_ID && !Soldo.softMG().canReceive(trx)) continue;
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Soldo.getEpochTime();
        ...
        for (Object transactionData : transactionsData) {
            try {
                ...
                if (transaction.getSenderId() == Genesis.CREATOR_ID) {
                    ...
                    if (transaction.getFeeNQT() < Soldo.softMG().getFixedFee(transaction.getAmountNQT())) {
                        continue;
                    }
                    if (transaction.getSenderId() == transaction.getRecipientId()) {
