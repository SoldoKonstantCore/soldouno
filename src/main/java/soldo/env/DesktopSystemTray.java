...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/ui/img/soldo-icon-32x32.png", "tray icon");
        ...
        MenuItem launchSoldoAerial = new MenuItem("Launch Aerial");
        if (!Soldo.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("soldodesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("soldodesktop.DesktopApplication failed to launch", exception);
            }
        });

        showLightWallet.addActionListener(e -> {
            try {
                Class.forName("soldodesktop.DesktopApplication").getMethod("lightlaunch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("soldodesktop.DesktopApplication failed to launch", exception);
            }
        });
        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("soldodesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("soldo desktop.DesktopApplication failed to refresh", exception);
            }
        });
        launchSoldoAerial.addActionListener(e -> {
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", Soldo.APPLICATION);
        addDataRow(statusPanel, "Version", Soldo.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(Soldo.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(Soldo.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", Soldo.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "SOLDO Server Status");
