...
/**
 * Handle logging for the Soldo node server
 */
public final class Logger {
     ...
     * The soldo/conf/logging-default.properties and soldo/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "soldo.util.SoldoLogManager");
        if (!(LogManager.getLogManager() instanceof SoldoLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (!Boolean.getBoolean("soldo.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Soldo.loadProperties(loggingProperties, "logging-default.properties", true);
                Soldo.loadProperties(loggingProperties, "logging.properties", false);
                Soldo.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(soldo.Soldo.class);
        enableStackTraces = Soldo.getBooleanProperty("soldo.enableStackTraces");
        enableLogTraceback = Soldo.getBooleanProperty("soldo.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof SoldoLogManager) {
            ((SoldoLogManager) LogManager.getLogManager()).coreShutdown();
        }
    }
