...
import soldo.http.ParameterParser;
...
import static soldo.http.JSONResponses.ERROR_NOT_ALLOWED;

public class SoldoTree {

    public static final boolean     ALLOW_API_HIERARCHY_WITHOUT_PASSWORD =  Soldo.getBooleanProperty("soldo.allowAPIHierarchyWithoutPassword", false);
    public static final boolean     ALLOW_API_HIERARCHY_ONLY_LOCALHOST   =  Soldo.getBooleanProperty("soldo.allowAPIHierarchyOnlyLocalhost", true);
    ...
    public static Account getParent (Account a) {
        ...
        try {
            try (PreparedStatement statement = Soldo.softMG().getConnection().prepareStatement("select parent_id from soft where id=?")) {
    ...
    public static AccountMinimal getParentOf (long accountId) {
        ...
        if (parentID == 0L)
            return null;
        try {
            try (PreparedStatement statement = Soldo.softMG().getConnection().prepareStatement("select balance,amount from soft where id=?")) {
    ...
    public static List<AccountMinimal> getDirectChildrenOf (long accountId, int internalParentID, int internalIDStart, boolean limit, int startIndex, boolean orderByAmount) throws SQLException {
        ...
        try {
            try (PreparedStatement statement = Soldo.softMG().getConnection().prepareStatement("select id,balance,amount from soft where parent_id=?"  + (orderByAmount?" order by amount desc":"") + (limit?" limit 100"+(startIndex>0?" offset ?":""):"") )) {
                statement.setLong(1, accountId);
    ...
    public static int getChildCountOf (long accountId) {
        try {
            try (PreparedStatement statement = Soldo.softMG().getConnection().prepareStatement("select count(*) from soft where parent_id=?")) {
                statement.setLong(1, accountId);
                try (ResultSet rs = statement.executeQuery()) {
                    while (rs.next()) {
    ...
    public static class AccountMinimal {
        ...
        public AccountMinimal (long id, int internalID, int parentInternalID, long balance, long amount) throws Exception {
            ...
            this.amount = amount;
            this.forged = Soldo.getBlockchain().getBlockCount(id);
            if (id != 0L) {
    ...
    public static AccountLoyaltyContainer getMostLoyalParentFaster (Account account) {
        ...
        try (DbIterator<? extends Transaction> iterator = Soldo.getBlockchain().getTransactions(account.getId(), (byte) -1, (byte) -1, 0, false)) {
            while (iterator.hasNext()) {
                Transaction transaction = iterator.next();
