...
public class UPnP {
    ...
    public static synchronized void addPort(int port) {
        ...
        try {
            if (gateway.addPortMapping(port, port, localAddress.getHostAddress(), "TCP",
                                       Soldo.APPLICATION + " " + Soldo.VERSION)) {
    ...
    private static void init() {
        ...
        try {
            ...
            GatewayDevice.setHttpReadTimeout(Soldo.getIntProperty("soldo.upnpGatewayTimeout", GatewayDevice.getHttpReadTimeout()));
            GatewayDiscover discover = new GatewayDiscover();
            discover.setTimeout(Soldo.getIntProperty("soldo.upnpDiscoverTimeout", discover.getTimeout()));
