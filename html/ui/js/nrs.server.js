...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, isAsync) {
        ...
        try {
            var sldFields = [
                ...
                ["priceSOLDO", "priceNQT"],
                ["refundSOLDO", "refundNQT"],
                ["discountSOLDO", "discountNQT"],
                ["phasingQuorumSOLDO", "phasingQuorum"],
                ["phasingMinBalanceSOLDO", "phasingMinBalance"],
                ["controlQuorumSOLDO", "controlQuorum"],
                ["controlMinBalanceSOLDO", "controlMinBalance"],
                ["controlMaxFeesSOLDO", "controlMaxFees"],
                ["minBalanceSOLDO", "minBalance"],
                ["shufflingAmountSOLDO", "amount"],
                ["monitorAmountSOLDO", "amount"],
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToSOLDO(phasingControl.maxFees)
                    })
                });
                return;
            }
    ...
    function addAddressData(data) {
        if (typeof data == "object" && ("recipient" in data)) {
            ...
            if (/^SOLDO\-/i.test(data.recipient)) {
