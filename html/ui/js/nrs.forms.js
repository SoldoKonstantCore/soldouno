...
var NRS = (function(NRS, $) {
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("SOLDO", "").replace("NQT", "").capitalize();
				var value = $(this).val();
		...
		if (!data) {
        	...
        	data.feeSOLDO = data.amountSOLDO*0.01 < 0.150000 ? 0.150000 : ((data.amountSOLDO*0.01) < 100 ? (data.amountSOLDO*0.01) : 100 );
		data.feeSOLDO = isNaN(data.feeSOLDO) ? 0.150000 : data.feeSOLDO;
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				NRS.unlockForm($modal, $btn);
				return;
			} else if (!/^SOLDO\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^SOLDO\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
		...
		if (!NRS.showedFormWarning) {
			if ("amountSOLDO" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {

				try {
					var amountNQT = NRS.convertToNQT(data.amountSOLDO);
				} catch (err) {
			...
			if ("feeSOLDO" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {

				try {

					var feeNQT = NRS.convertToNQT(data.feeSOLDO);
			...
			var convertSOLDOFields = ["phasingQuorumSOLDO", "phasingMinBalanceSOLDO"];
			$.each(convertSOLDOFields, function(key, field) {
				if (field in data) {
    ...
    function updateFee(modal, feeNQT) {
        ...
        fee.val(NRS.convertToSOLDO(feeNQT));
        var recalcIndicator = $("#" + modal.attr('id').replace('_modal', '') + "_recalc");
