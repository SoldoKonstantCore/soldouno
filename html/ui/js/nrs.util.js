...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		...
		return NRS.convertToSOLDO(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};
    ...
    NRS.calculateOrderTotal = function (quantityQNT, priceNQT) {
        return NRS.convertToSOLDO(calculateOrderTotalImpl(quantityQNT, priceNQT));
    };
    ...
    NRS.convertToSOLDO = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = NRS.convertToSOLDO(amount, true);
    ...
    NRS.getAccountLink = function (object, accountKey, accountRef, title, showAccountRS, clazz) {
        ...
        if (accountRS.toUpperCase() === "SOLDO-5H5H-PRS4-UYTA-8GY83")
            return "GENESIS";
        var accountTitle;
    ...
    NRS.createInfoTable = function (data, fixed) {
		...
		for (var key in data) {
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                value = NRS.formatAmount(new BigInteger(String(value))) + " SOLDO";
            } else if (key == "sender" || key == "recipient" || key == "account" || key == "seller" || key == "buyer" || key == "lessee") {
                if (value.toUpperCase() === "SOLDO-5H5H-PRS4-UYTA-8GY83" || value === "6961384895484640367") {
                    value = "GENESIS";
                } else {
