 ...
 */
 var NRS = (function(NRS, $, undefined) {
	...
	NRS.showLoginScreen = function() {
        ...
        $("#login_account_other").mask("SOLDO-****-****-****-*****");
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedSoldoAccounts") && NRS.getStrItem("savedSoldoAccounts") != ""){
			...
			var accounts = NRS.getStrItem("savedSoldoAccounts").split(";");
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedSoldoAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedSoldoAccounts');
		} else {
			NRS.setStrItem("savedSoldoAccounts", accounts);
		}
		NRS.listAccounts();
	};
		...
		NRS.sendRequest("getBlockchainStatus", {}, function(response) {
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					$("#sidebar_account_id").html("SOLDO-<br/>"+String(NRS.accountRS).escapeHTML().substring(6));

					/* Left Panel Copy Account Info Buttons */
					$('.soldo-main .left-panel .card .address a').html(String(NRS.accountRS).escapeHTML());
            ...
            try {
              ...
              console.log("Error during fork check: "+err.message);
					...
					if ($("#remember_account").is(":checked") || NRS.newlyCreatedAccount) {
						var accountExists = 0;
						if (NRS.getStrItem("savedSoldoAccounts")) {
							accounts = NRS.getStrItem("savedSoldoAccounts").split(";");
							$.each(accounts, function(index, account) {
						...
						if (!accountExists){
							if (NRS.getStrItem("savedSoldoAccounts") && NRS.getStrItem("savedSoldoAccounts") != ""){
								accounts = NRS.getStrItem("savedSoldoAccounts") + NRS.accountRS + ";";
								NRS.setStrItem("savedSoldoAccounts", accounts);
							} else {
								NRS.setStrItem("savedSoldoAccounts", NRS.accountRS + ";");
							}
						}
					}
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your SOLDO passphrase.";
			html += "</p>";
			...
			html += "you can loose your SOLDO! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of SOLDO now or in the future.";
			html += "</p>";
			html += "</div>";
