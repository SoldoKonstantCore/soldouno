...
var NRS = (function(NRS, $) {
    ...
    try {
      if (String(account.toUpperCase()) === "SOLDO-5H5H-PRS4-UYTA-8GY83" || String(account) === "6961384895484640367")
        return;
    } catch (err) {}
    ...
    if (NRS.discoveryState == NRS.DISCOVERY_PUBLIC) {
      NRS.sendRequest("getParent", {"account" : NRS.userInfoModal.user }, function(response) {
        ...
        if (response.parentRS) {
          if (response.parentRS.toUpperCase() == "SOLDO-5H5H-PRS4-UYTA-8GY83") {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			...
			$("#user_info_modal_account_balance").html(NRS.formatAmount(account.unconfirmedBalanceNQT) + " SOLDO");
		}
  ...
  userInfoModal.on("hidden.bs.modal", function() {
    ...
    function getTransactionType(transaction) {
        var transactionType = $.t(NRS.transactionTypes[transaction.type].subTypes[transaction.subtype].i18nKeyTitle);
				if (transaction.senderRS == "SOLDO-5H5H-PRS4-UYTA-8GY83") {
    ...
    NRS.sendRequest("getBlockchainTransactions", {
          ...
          var receiving;
					if (/^SOLDO\-/i.test(String(NRS.userInfoModal.user))) {
